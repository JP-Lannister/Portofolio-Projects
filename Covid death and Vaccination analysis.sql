...
-- and dea.location = 'Nigeria'
 ...
 -- View showing deathrate due to Covid in Nigeria

Create View
Coviddeathrate_Nigeria as
select location, date, total_cases, total_deaths, ((total_deaths/total_cases)*100) as Death_Percentage
from Portofolioproject.dbo.['owid-covid-data (1)$']
where location = 'Nigeria'


-- Showing the rate of covid infection in Nigeria (View)

Create View
Covidinfectionrate_Nigeria as
select location, date, total_cases, population, ((total_cases/population)*100) as Percentage_infectionRate
from Portofolioproject.dbo.['owid-covid-data (1)$']
where location = 'Nigeria'


-- Infection rate by population per countries (View)

Create View
GlobalInfectionRate as
select location, population, max(total_cases) as Highest_InfectionCount, max((total_cases/population)*100) as
Percentage_population_Infected
from Portofolioproject.dbo.['owid-covid-data (1)$']
group by location, population
...
-- Total death count around the world (View)

Create view
Global_Deathcount as
select location, max(cast(total_deaths as int)) as Total_BodyCount
from Portofolioproject.dbo.['owid-covid-data (1)$']
where location is not null and
location <> ('Upper middle income') and location <> ('High income') and location <> ('lower middle income') 
group by location
  


-- Vaccination Levels in Nigeria (View)
...
-- Global vaccination Levels (View)
